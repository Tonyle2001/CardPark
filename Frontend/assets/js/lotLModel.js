function LotLModel() {
    this.spots = JSON.parse(localStorage.getItem('parkingSpots')) || new Array(14).fill({occupied: false, timeoutEnd: null});
    fetch('http://localhost:3000/reservations/lot/L')
    .then(response =>{
        if (!response.ok) {
...
LotLModel.prototype.setOccupied = function(index) {
...
LotLModel.prototype._startTimeout = function(index, delay) {
    setTimeout(() => {
...
LotLModel.prototype._commit = function() {
    localStorage.setItem('parkingSpots', JSON.stringify(this.spots));
};

LotLModel.prototype.restoreState = function() {
