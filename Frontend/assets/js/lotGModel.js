function LotGModel() {
    this.spots = JSON.parse(localStorage.getItem('parkingSpots')) || new Array(42).fill({occupied: false, timeoutEnd: null});
    fetch('http://localhost:3000/reservations/lot/G')
    .then(response =>{
        if (!response.ok) {
...
LotGModel.prototype.setOccupied = function(index) {
...
LotGModel.prototype._startTimeout = function(index, delay) {
    setTimeout(() => {
...
LotGModel.prototype._commit = function() {
    localStorage.setItem('parkingSpots', JSON.stringify(this.spots));
};

LotGModel.prototype.restoreState = function() {
