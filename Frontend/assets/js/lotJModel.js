function LotJModel() {
    this.spots = JSON.parse(localStorage.getItem('parkingSpots')) || new Array(156).fill({occupied: false, timeoutEnd: null});
    fetch('http://localhost:3000/reservations/lot/J')
    .then(response =>{
        if (!response.ok) {
...
LotJModel.prototype.setOccupied = function(index) {
...
LotJModel.prototype._startTimeout = function(index, delay) {
    setTimeout(() => {
...
LotJModel.prototype._commit = function() {
    localStorage.setItem('parkingSpots', JSON.stringify(this.spots));
};

LotJModel.prototype.restoreState = function() {
