...
router.get('/', function(req, res) {
    ...
    dbConn.query('SELECT * FROM Reservations',function(err,reservations){

        if(err) {
          res.status(500).send({ message: err.message });
        } else {
            res.status(200).send({ users:reservations });
        }
    });
});
...
router.get('/:uid', function(req, res) {
  ...
  dbConn.query('SELECT * FROM Reservation where uid = ?', uid, function(err, result){
